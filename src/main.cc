...
static void convertColor(const zutty::Color &color, XColor &xcolor) {
...
static void setXCursor() {
  ...
  const static unsigned char cursorBits[] = {
      0x00, 0x00, 0xee, 0x00, 0x38, 0x00, 0x10, 0x00, 0x10, 0x00, 0x10,
      0x00, 0x10, 0x00, 0x10, 0x00, 0x10, 0x00, 0x10, 0x00, 0x10, 0x00,
      0x10, 0x00, 0x10, 0x00, 0x38, 0x00, 0xee, 0x00, 0x00, 0x00};
  const static unsigned char cursorMaskBits[] = {
      0xef, 0x01, 0xff, 0x01, 0xff, 0x01, 0x7c, 0x00, 0x38, 0x00, 0x38,
      0x00, 0x38, 0x00, 0x38, 0x00, 0x38, 0x00, 0x38, 0x00, 0x38, 0x00,
      0x38, 0x00, 0x7c, 0x00, 0xff, 0x01, 0xff, 0x01, 0xef, 0x01};
  ...
  Pixmap pxcur = XCreateBitmapFromData(xDisplay, root, (char *)cursorBits,
                                       cursorW, cursorH);
  Pixmap pxmask = XCreateBitmapFromData(xDisplay, root, (char *)cursorMaskBits,
                                        cursorW, cursorH);
  Cursor cursor = XCreatePixmapCursor(xDisplay, pxcur, pxmask, &xc_fg, &xc_bg,
                                      cursorHotX, cursorHotY);
...
static void makeXWindow(const char *name, int width, int height, int px, int py,
                        EGLDisplay eglDpy, EGLContext &eglCtx,
  ...
  xWindow =
      XCreateWindow(xDisplay, root, 0, 0, width, height, 0, visInfo->depth,
                    InputOutput, visInfo->visual, mask, &attr);
  logI << "Window ID: " << xWindow << " / 0x" << std::hex << xWindow << std::dec
       << std::endl;
...

  {
    ...
    XmbSetWMProperties(xDisplay, xWindow, name, name, nullptr, 0, &sizeHints,
                       &wmHints, &classHint);
  }

  {
    ...
    pid_t pid = getpid();
    XChangeProperty(xDisplay, xWindow, _NET_WM_PID, XA_CARDINAL, 32,
...

  {
    ...
    XChangeProperty(xDisplay, xWindow, _NET_WM_ICON, XA_CARDINAL, 32,
                    PropModeReplace, (const unsigned char *)wm_icons,
...
static void resolveShell(char *progPath) {
  char resolvedPath[PATH_MAX];
  if (progPath[0] == '/')
    return; // absolute path; we are done
  else if (progPath[0] == '.' && realpath(progPath, resolvedPath) != nullptr) {
...
static void validateShell(char *progPath) {
...
static int startShell(const char *execPath, const char *const argv[]) {
  int ptyFd;
  pid_t pid;
...
static VtModifier convertKeyState(KeySym ks, unsigned int state) {
...
static void pasteCb(bool success, const std::string &content) {
  if (success)
    vt->pasteSelection(content);
}

static bool onKeyPress(XEvent &event, XIC &xic, int ptyFd) {
  using Key = VtKey;
  ...
  if (xic) {
    ...
    if (status == XBufferOverflow) {
      logE << "KeyPress event: buffer size " << sizeof(buffer)
           ...
           << " a buffer with " << nbytes + 1 << " bytes." << std::endl;
      return true;
    }
  } else {
...
static inline bool isMouseProtocol(unsigned int state,
                                   const MouseTrackingState &mouseTrk) {
  return !mouseCtx.selectionOngoing && !(state & ShiftMask) &&
         mouseTrk.mode != MouseTrackingMode::Disabled;
}

static inline void mouseProtoSend(MouseTrackingEnc enc, int eventType,
                                  unsigned int modstate, int button, int cx,
                                  int cy) {
  int cb = 0;
  if (eventType == MotionNotify) {
    if (modstate & Button1Mask)
      cb = 32;
    else if (modstate & Button2Mask)
      cb = 33;
    else if (modstate & Button3Mask)
      cb = 34;
    else
      cb = 35;
...
static inline void mouseProtoConvCoords(int px, int py, uint16_t &out_cx,
                                        uint16_t &out_cy) {
  out_cx = std::max(0, (px - opts.border - 1) / fontpk->getPx()) + 1;
  out_cy = std::max(0, (py - opts.border - 1) / fontpk->getPy()) + 1;
}

static inline void onButtonPressMouseProto(XButtonEvent &xbevt,
                                           const MouseTrackingState &mouseTrk) {
  uint16_t cx, cy;
  ...
  case MouseTrackingMode::VT200_AnyEvent:
    ...
    mouseProtoSend(mouseTrk.enc, ButtonPress, xbevt.state, xbevt.button, cx,
                   cy);
    break;
...
onButtonReleaseMouseProto(XButtonEvent &xbevt,
  ...
  case MouseTrackingMode::VT200_AnyEvent:
    mouseProtoConvCoords(xbevt.x, xbevt.y, cx, cy);
    mouseProtoSend(mouseTrk.enc, ButtonRelease, xbevt.state, xbevt.button, cx,
                   cy);
    break;
  case MouseTrackingMode::X10_Compat:
  case MouseTrackingMode::Disabled:
...
onMotionNotifyMouseProto(XMotionEvent &xmoevt,
  ...
  case MouseTrackingMode::VT200_AnyEvent:
    ...
    if (cx != lastCx || cy != lastCy) {
      mouseProtoSend(mouseTrk.enc, MotionNotify, xmoevt.state, 0, cx, cy);
      lastCx = cx;
      lastCy = cy;
    }
    break;
...
static void onButtonPress(XButtonEvent &xbevt, bool &holdPtyIn) {
...
static void onButtonRelease(XButtonEvent &xbevt, bool &holdPtyIn) {
  const auto &mouseTrk = vt->getMouseTrackingState();
  if (isMouseProtocol(xbevt.state, mouseTrk)) {
...
static void onMotionNotify(XMotionEvent &xmoevt) {
  const auto &mouseTrk = vt->getMouseTrackingState();
...
static bool x11Event(XEvent &event, XIC &xic, int ptyFd, bool &destroyed,
                     bool &holdPtyIn) {
...
static bool eventLoop(XIC &xic, int ptyFd) {
...
static void handleOsc(int cmd, const std::string &arg) {
  switch (cmd) {
  ...
  case 52: // Manipulate Selection Data
  {
    ...
    if (p == std::string::npos) {
      logT << "Malformed argument to OSC 52 (missing ';'): '" << arg << "'"
           << std::endl;
      break;
    }
...
static void printGLInfo(EGLDisplay eglDpy) {
...
static int handleXIOError(Display *dpy) {
  int err = errno;
  logE << "Fatal IO error " << err << " (" << strerror(err) << ") on X server "
       << DisplayString(dpy) << std::endl;
...
extern "C" int old_main(int argc, char *argv[]) {
  ...
  fontpk =
      std::make_unique<Fontpack>(opts.fontpath, opts.fontname, opts.dwfontname);

  int winWidth = 2 * opts.border + opts.nCols * fontpk->getPx();
  int winHeight = 2 * opts.border + opts.nRows * fontpk->getPy();

  makeXWindow(opts.title, winWidth, winHeight, fontpk->getPx(), fontpk->getPy(),
              eglDpy, eglCtx, eglSurface);

  XMapWindow(xDisplay, xWindow);

  if (xim && ximStyle) {
    xic = XCreateIC(xim, XNInputStyle, ximStyle, XNClientWindow, xWindow,
                    XNFocusWindow, xWindow, nullptr);
  ...
  renderer = std::make_unique<Renderer>(
      ...
      [eglDpy, eglSurface]() { eglSwapBuffers(eglDpy, eglSurface); },
      fontpk.get());

  setupSignals();
  int ptyFd = startShell(progPath, shArgv);
  vt = std::make_unique<Vterm>(fontpk->getPx(), fontpk->getPy(), winWidth,
                               winHeight, ptyFd);
  vt->setRefreshHandler([](const zutty::Frame &f) { renderer->update(f); });
  vt->setOscHandler(
      [](int cmd, const std::string &arg) { handleOsc(cmd, arg); });
