...
int saveCandidate(const char *fpath, const char *ext, int level,
...
int fontFileFilter(const char *fpath, const struct stat *sb, int tflag,
                   struct FTW *ftwbuf) {
  // If we have just emerged from a directory where fonts were found,
  // time to call it a day.
  if (tflag == FTW_D && sstate.level > 0 && ftwbuf->level == sstate.level - 1 &&
  ...
  if (strcasecmp(ext, ".ttc") != 0 && strcasecmp(ext, ".ttf") != 0 &&
      strcasecmp(ext, ".otf") != 0 && strcasecmp(ext, ".pcf") != 0 &&
  ...
  if (midlen > 0 && (mid[0] == '-' || mid[0] == '_' || mid[0] == ' ')) {
    ++mid;
    --midlen;
  }

  // Apply some simple heuristics to identify font variants
  if (midlen == 0 || strncasecmp(mid, "R", midlen) == 0 ||
      strncasecmp(mid, "Regular", midlen) == 0) {
    if (saveCandidate(fpath, ext, ftwbuf->level, "Regular", sstate.regular))
      return 0;
  } else if (strncasecmp(mid, "B", midlen) == 0 ||
             strncasecmp(mid, "Bold", midlen) == 0) {
    if (saveCandidate(fpath, ext, ftwbuf->level, "Bold", sstate.bold))
      return 0;
    ...
    if (saveCandidate(fpath, ext, ftwbuf->level, "Italic", sstate.italic))
      return 0;
  } else if (strncasecmp(mid, "BI", midlen) == 0 ||
             strncasecmp(mid, "BoldIt", midlen) == 0 ||
             strncasecmp(mid, "BoldItalic", midlen) == 0) {
    if (saveCandidate(fpath, ext, ftwbuf->level, "BoldItalic",
                      sstate.boldItalic))
      return 0;
  }
...
namespace zutty {
Fontpack::Fontpack(const std::string &fontpath, const std::string &fontname,
                   const std::string &dwfontname) {
  logT << "Fontpack: fontpath=" << fontpath << "; fontname=" << fontname
  ...
  do {
    ...
    size_t len =
        (nextpos == std::string::npos) ? std::string::npos : nextpos - pos;
  ...
  if (!sstate.regular.size()) {
    ...
    throw std::runtime_error(std::string("No suitable files for '") + fontname +
                             "' found!");
  }
  ...
  try {
    if (sstate.bold.size())
      fontBold = std::make_unique<Font>(sstate.bold, *fontRegular.get(),
                                        Font::Overlay);
  ...
  try {
    if (sstate.italic.size())
      fontItalic = std::make_unique<Font>(sstate.italic, *fontRegular.get(),
                                          Font::Overlay);
  } catch (const std::runtime_error &e) {
    fontItalic = nullptr;
  ...
  try {
    if (sstate.regular.size())
      ...
      logW << "Failed to locate requested double-width font: " << dwfontname
           << std::endl;
    }
  } catch (const std::runtime_error &e) {
