...
public:
  ...
  Frame(uint16_t winPx_, uint16_t winPy_, uint16_t nCols_, uint16_t nRows_,
        uint16_t &marginTop_, uint16_t &marginBottom_, uint16_t saveLines_ = 0);

  void resize(uint16_t winPx_, uint16_t winPy_, uint16_t nCols_,
              uint16_t nRows_, uint16_t &marginTop_, uint16_t &marginBottom_);
  ...
  void moveInRow(uint16_t pY, uint16_t dstX, uint16_t srcX, uint16_t count);
  void copyRow(uint16_t dstY, uint16_t srcY, uint16_t startX, uint16_t count);
  ...
  // selection state
  enum class SelectSnapTo : uint8_t { Char = 0, Word, Line, COUNT };
...
private:
  ...
  void eraseRange(uint32_t start, uint32_t end, const CharVdev::Cell &attrs);
  ...
  static SelectSnapTo cycleSelectSnapTo(SelectSnapTo &snapTo) {
    return static_cast<SelectSnapTo>((static_cast<uint8_t>(snapTo) + 1) %
