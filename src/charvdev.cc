...
GLuint createShader(GLuint type, const char *src, const char *name) {
  GLint stat;
...
void setupTexture(GLuint target, GLenum type, GLuint &texture) {
  if (texture) {
...
void setupAtlasTexture(const zutty::Font &fnt, int idx) {
...
void setupAtlasMappingTexture(const zutty::Font &fnt, GLuint target,
                              GLuint &texture) {
  ...
  {
    auto rcIt = fnt.getAtlasMap().find(zutty::Unicode_Replacement_Character);
    if (rcIt != itEnd)
      apRC = rcIt->second;
  }
  ...
  for (int k = 0; k < 256 * 256; ++k) {
    const auto &apos =
        ((k >= 0xd800 && k < 0xe000) || k >= 0xfffe) ? apRC : apMG;
    atlasMap[2 * k] = apos.x;
...
template <typename T>
void setupStorageBuffer(GLuint index, GLuint &buffer, uint32_t n_items) {
  if (buffer) {
...
CharVdev::CharVdev(Fontpack *fontpk)
    : px(fontpk->getPx()), py(fontpk->getPy()) {
  createShaders();
  ...
  static const GLfloat verts[4][2] = {{-1, 1}, {1, 1}, {-1, -1}, {1, -1}};
  static const GLfloat texCoords[4][2] = {{0, 0}, {1, 0}, {0, 1}, {1, 1}};

  glVertexAttribPointer(A_pos, 2, GL_FLOAT, GL_FALSE, 0, verts);
  glVertexAttribPointer(A_vertexTexCoord, 2, GL_FLOAT, GL_FALSE, 0, texCoords);
  glCheckError();
  ...
  glTexStorage3D(GL_TEXTURE_2D_ARRAY, 1, GL_R8, reg.getPx() * reg.getNx(),
                 reg.getPy() * reg.getNy(),
                 4); // number of layers
  glCheckError();
  ...
  if (fontpk->hasDoubleWidth()) {
    ...
    glTexStorage3D(GL_TEXTURE_2D_ARRAY, 1, GL_R8, dw.getPx() * dw.getNx(),
                   dw.getPy() * dw.getNy(),
                   1); // number of layers
    glCheckError();
...
bool CharVdev::resize(uint16_t pxWidth_, uint16_t pxHeight_) {
  ...
  logI << "Resize to " << pxWidth << " x " << pxHeight << " pixels, " << nCols
       << " x " << nRows << " chars" << std::endl;

  GLint viewWidth = nCols * px;
  GLint viewHeight = nRows * py;
  glViewport(opts.border, pxHeight - viewHeight - opts.border, viewWidth,
             viewHeight);
  ...
  glBindImageTexture(0, T_output, 0, GL_FALSE, 0, GL_WRITE_ONLY, GL_RGBA32F);
  glCheckError();
...
void CharVdev::setCursor(const Cursor &cursor) {
...
void CharVdev::setSelection(const Rect &sel) {
  static Rect prev;
...
void CharVdev::setDeltaFrame(bool delta) {
  glUseProgram(P_compute);
...
void CharVdev::draw() {
  ...
  glUseProgram(P_draw);
  glClearColor(opts.bg.red / 255.0, opts.bg.green / 255.0, opts.bg.blue / 255.0,
               1.0);
...
CharVdev::Mapping::Mapping(uint16_t nCols_, uint16_t nRows_, Cell *&cells_)
    : nCols(nCols_), nRows(nRows_), cells(cells_){};
...
CharVdev::Mapping CharVdev::getMapping() {
  ...
  cells = reinterpret_cast<Cell *>(glMapBufferRange(
...
void CharVdev::createShaders() {
  GLuint S_compute, S_fragment, S_vertex;

  S_compute = createShader(GL_COMPUTE_SHADER, computeShaderSource, "compute");
  S_fragment =
      createShader(GL_FRAGMENT_SHADER, fragmentShaderSource, "fragment");
  S_vertex = createShader(GL_VERTEX_SHADER, vertexShaderSource, "vertex");
  ...
  logT << "compute program:"
       ...
       << " deltaFrame=" << compU_deltaFrame << " showWraps=" << compU_showWraps
       << " hasDoubleWidth=" << compU_hasDoubleWidth << std::endl;
  ...
  logT << "draw program:"
       << " attrib pos=" << A_pos << " vertexTexCoord=" << A_vertexTexCoord
       << " uniform viewPixels=" << drawU_viewPixels << std::endl;
}
