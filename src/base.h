...
struct Rect {
  ...
  Rect(int x, int y) : tl(x, y), br(x + 1, y) {}
  Rect(int x1, int y1, int x2, int y2) : tl(x1, y1), br(x2, y2) {}

  bool null() const { return tl == Point() && br == Point(); }

  bool empty() const { return tl == br; }

  Point mid() const { return Point((tl.x + br.x) / 2, (tl.y + br.y) / 2); }
  ...
  void toggleRectangular() { rectangular = !rectangular; }
};
