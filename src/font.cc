...
namespace zutty {
Font::Font(const std::string &filename_) : filename(filename_), overlay(false) {
  load();
}

Font::Font(const std::string &filename_, const Font &priFont, Overlay_)
    : filename(filename_), overlay(true), px(priFont.getPx()),
      py(priFont.getPy()), baseline(priFont.getBaseline()), nx(priFont.getNx()),
      ny(priFont.getNy()), atlasBuf(priFont.getAtlas()),
      atlasMap(priFont.getAtlasMap()) {
  load();
}

Font::Font(const std::string &filename_, const Font &priFont, DoubleWidth_)
    : filename(filename_), dwidth(true), px(2 * priFont.getPx()),
      py(priFont.getPy()) {
  load();
}
...
void Font::load() {
  ...
  logT << "Family: " << face->family_name << "; Style: " << face->style_name
       << "; Faces: " << face->num_faces << "; Glyphs: " << num_glyphs
       << " to load (" << face->num_glyphs << " total)" << std::endl;
  ...
  if (!overlay) {
    ...
    logT << "Atlas texture geometry: " << nx << "x" << ny << " glyphs of " << px
         << "x" << py << " each, "
         << "yielding pixel size " << nx * px << "x" << ny * py << "."
         << std::endl;
    logT << "Atlas holds space for " << nx * ny << " glyphs, " << n_glyphs
         << " will be used, empty: " << nx * ny - n_glyphs << " ("
         << 100.0 * (nx * ny - n_glyphs) / (nx * ny) << "%)" << std::endl;
...
void Font::loadFixed(const FT_Face &face) {
  ...
  int bestHeightDiff = std::numeric_limits<int>::max();
  {
    ...
    for (int i = 0; i < face->num_fixed_sizes; ++i) {
      oss << " " << face->available_sizes[i].width << "x"
  ...
  logT << "Configured size: " << (int)opts.fontsize
       << "; Best matching fixed size: " << face->available_sizes[bestIdx].width
       << "x" << face->available_sizes[bestIdx].height << std::endl;
...
void Font::loadScaled(const FT_Face &face) {
  ...
  double tpx =
      opts.fontsize * (double)face->max_advance_width / face->units_per_EM;
...
void Font::loadFace(const FT_Face &face, FT_ULong c, const AtlasPos &apos) {
  ...
  {
    for (int j = 0; j < bh; ++j) {
      uint8_t *atl_dst_row = atlasBuf.data() + atlas_glyph_offset + j * nx * px;
      for (int k = 0; k < bw; ++k) {
        *atl_dst_row++ = 0;
      }
    }
  ...
  case FT_PIXEL_MODE_GRAY:
    ...
    break;
  default:
    throw std::runtime_error(std::string("Unhandled pixel_type=") +
