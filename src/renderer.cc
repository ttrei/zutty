...
Renderer::Renderer(const std::function<void()> &initDisplay,
                   const std::function<void()> &swapBuffers_, Fontpack *fontpk)
...
void Renderer::renderThread(const std::function<void()> &initDisplay,
                            Fontpack *fontpk) {
  initDisplay();
  ...
  while (1) {
    std::unique_lock<std::mutex> lk(mx);
    cond.wait(lk, [&]() { return lastFrame.seqNo != nextFrame.seqNo; });
