...
constexpr VtModifier operator|(VtModifier m1, VtModifier m2) {
  return static_cast<VtModifier>(static_cast<uint8_t>(m1) |
...
enum class MouseTrackingEnc : uint8_t { Default = 0, UTF8, SGR, URXVT };
...
class Vterm {
public:
  Vterm(uint16_t glyphPx, uint16_t glyphPy, uint16_t winPx, uint16_t winPy,
        int ptyFd);
  ...
  struct InputSpec {
    ...
    size_t getLength() const { return length ? length : strlen(input); }
  };
...
private:
  ...
  const char *strInputState(InputState is) {
    static const char *enumerators[] = {
        "Normal",   "Escape",    "Escape_VT52",   "Esc_SPC",
        "Esc_Hash", "Esc_Pct",   "SelectCharset", "CSI",
        "CSI_priv", "CSI_Quote", "CSI_DblQuote",  "CSI_Bang",
        "CSI_SPC",  "CSI_GT",    "DCS",           "DCS_Esc",
        "OSC",      "OSC_Esc",   "VT52_CUP_Arg1", "VT52_CUP_Arg2"};
  ...
  enum class CompatibilityLevel : uint8_t { VT52, VT100, VT400 };
  CompatibilityLevel compatLevel = CompatibilityLevel::VT400;

  enum class CursorKeyMode : uint8_t { ANSI, Application };
  CursorKeyMode cursorKeyMode = CursorKeyMode::ANSI;

  enum class KeypadMode : uint8_t { Normal, Application };
  KeypadMode keypadMode = KeypadMode::Normal;

  enum class OriginMode : uint8_t { Absolute, ScrollingRegion };
  OriginMode originMode = OriginMode::Absolute;

  enum class ColMode : uint8_t { C80, C132 };
